  ...
  if (variant === "compact") {
    return (
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-3 flex items-center gap-2">
        <AirportPicker label="Depuis" value={origin} onChange={handleOriginChange} exclude={dest} lang={lang} />
        ...
        <AirportPicker label="Vers" value={dest} onChange={setDest} exclude={origin} lang={lang} />
        <button
          onClick={handleSearch}
          disabled={!canSearch}
          className="bg-primary text-white px-4 py-2 rounded-xl text-sm font-semibold disabled:opacity-40 hover:bg-[#1D4ED8] transition-colors flex-shrink-0"
        >
          Chercher
        </button>
      </div>
