        ...
        <div className="bg-white rounded-3xl shadow-2xl p-5 mb-5">
          ...
          <div className={`grid gap-3 mb-4 ${isOneWay ? "grid-cols-1" : "grid-cols-2"}`}>
            <div>
              ...
              <input type="date" value={depDate} min={addDays(today, 0)} onChange={e => setDepDate(e.target.value)}
            ...
            <div className="mb-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-3 text-xs">
                  <span className={`${gLoading ? "text-blue-300 animate-pulse" : googleFlights || gError ? "text-emerald-400" : "text-indigo-500"}`}>
                    {gLoading ? "🔵 Google..." : googleFlights ? "✅ Google" : gError ? "❌ Google" : ""}
                  </span>
                  <span className={`${sLoading ? "text-orange-300 animate-pulse" : skyFlights || sError ? "text-emerald-400" : "text-indigo-500"}`}>
              ...
              {allFlights.length > 0 && (
                <div className="space-y-2">
                  ...
                  {selectedIdx !== null && <p className="text-center text-indigo-400 text-xs py-1">Prix sélectionné utilisé pour la comparaison miles ↓</p>}
                </div>
              )}
